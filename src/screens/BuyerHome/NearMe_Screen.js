import React, { useState, createRef, useEffect, useRef } from 'react';
import Header from '../../components/NotificationHeader/NotificationHeader.Component';
import MapView from "react-native-map-clustering";
import { Marker, PROVIDER_GOOGLE, Circle } from 'react-native-maps';
import { width, height, totalSize } from 'react-native-dimension';
import Geolocation from '@react-native-community/geolocation';
import { PERMISSIONS, requestMultiple, request,openSettings } from 'react-native-permissions';
import Colors_1 from '../../utills/Colors';

import dummyData from './DummyData';

import { useSelector } from 'react-redux';
import {
    SafeAreaView,
  TouchableOpacity,
  Image,
  View,
  Platform,
  Alert,
  TextInput,
  Text,
} from 'react-native';
import { Colors } from 'react-native/Libraries/NewAppScreen';
import Icon_1 from 'react-native-vector-icons/MaterialCommunityIcons';
import Container from '../../components/MapsContainer/MapsContainer.component';
import { ScrollView } from 'react-native';

function distance(lat1, lon1, lat2, lon2, unit) {
    if ((lat1 == lat2) && (lon1 == lon2)) {
      return 0;
    }
    else {
      var radlat1 = Math.PI * lat1 / 180;
      var radlat2 = Math.PI * lat2 / 180;
      var theta = lon1 - lon2;
      var radtheta = Math.PI * theta / 180;
      var dist = Math.sin(radlat1) * Math.sin(radlat2) + Math.cos(radlat1) * Math.cos(radlat2) * Math.cos(radtheta);
      if (dist > 1) {
        dist = 1;
      }
      dist = Math.acos(dist);
      dist = dist * 180 / Math.PI;
      dist = dist * 60 * 1.1515;
      dist = dist * 1.609344
      dist = dist * 1000;
      return dist;
    }
  }

const NearMe_Screen = ({ route, navigation }) => {
 /**
   * Reading Screen Data from redux depending on selected language
   */
  const languageJson = useSelector(state => state.Language.languageJson.Map_screen)
  const selectdLanguage = useSelector(state => state.Language.selectdLanguage)
  const selectedDistanceUnit = useSelector(state => state.Configuration.selectedDistanceUnit)
  const RADIUS = useSelector(state => state.Configuration.selectedRadiusInMeters)
  const providersFilter = useSelector(state => state.Configuration.showProviders)
  useEffect(() => {
    // console.log("Buyer Maps:", providersFilter)
  }, [])
  /**
   * States for user location and markers
   */
  const [markers, setMarkers] = useState(dummyData.feature.map(item => item.location));
  const [userLocation, setUserLocation] = useState({
    latitude: 37.7853621,
      longitude: -122.4005841
  });
  const [location, setLocation] = useState({
    latitude: 37.7853621,
    longitude: -122.4005841
  });
  const regionFrom = (distance) => {
    distance = distance
    const circumference = 40075
    const oneDegreeOfLatitudeInMeters = 111.32 * 1000
    const angularDistance = distance / circumference
    const latitudeDelta = distance / oneDegreeOfLatitudeInMeters
    const longitudeDelta = Math.abs(Math.atan2(
      Math.sin(angularDistance) * Math.cos(userLocation.latitude),
      Math.cos(angularDistance) - Math.sin(userLocation.latitude) * Math.sin(userLocation.latitude)))
    setLatDelta(latitudeDelta)
    setLonDelta(longitudeDelta)

    mapRef.current.animateToRegion({
      ...userLocation, 
      latitudeDelta: latitudeDelta,
      longitudeDelta: longitudeDelta,
    }, 1000)
  }
  /**
   * Setting Markers state at the start of the screen
   */
  useEffect(() => {
    // var marker = dummyData.feature.map(item => item.location);
    // setMarkers([...marker]);
    regionFrom(RADIUS)
  }, [RADIUS]);
  const [filterVisible, setFilterVisible] = useState(false)
  const [latDelta, setLatDelta] = useState(0.3)
  const [lonDelta, setLonDelta] = useState(0.3)
  /**
   * Create refernce for description textinput and search input
   */
  var tempMarker = markers.filter(item => {
    const dis = distance(location.latitude, location.longitude,)
    if (dis < RADIUS)
      return true
    else false
  })
  var tempData = dummyData.feature.filter(item => {
    var valid = false
    tempMarker.map(ite => {
      if (ite == item.location) { valid = true }
    })
    return valid
  })
  const sliderRef = useRef(null)
  useEffect(() => {
    
    setInterval(() => {
      circle.current && circle.current.setNativeProps({ fillColor: 'rgba(230,238,255,0.5)', strokeColor: Colors_1.buttonBlue });
    }, 10);
    // getLocation()
  }, [])
  /*
  *  getting current location of user
  */
  const getLocation = async () => {
    var loc = {}
    try {
      requestMultiple(
        Platform.select({
          ios: [PERMISSIONS.IOS.LOCATION_ALWAYS, PERMISSIONS.IOS.LOCATION_WHEN_IN_USE],
          android: [PERMISSIONS.ANDROID.ACCESS_COARSE_LOCATION,
          PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION,
          PERMISSIONS.ANDROID.ACCESS_BACKGROUND_LOCATION],
        })).then((statuses) => {
          if (statuses[PERMISSIONS.IOS.LOCATION_WHEN_IN_USE] == 'granted' || statuses[PERMISSIONS.IOS.LOCATION_WHEN_IN_USE] == 'granted') {
            Geolocation.getCurrentPosition(info => {
              loc = {
                latitude: info.coords.latitude,
                longitude: info.coords.longitude,
              }
              setLocation(loc)
              setUserLocation(loc)
              mapRef.current.animateToRegion({
                ...loc,
                 latitudeDelta: 0.3,
                longitudeDelta: 0.3,
              }, 1000)  
            }).catch(e => console.log(e))
          }
          else {
            request(PERMISSIONS.IOS.LOCATION_ALWAYS).then((result) => {
             if (result=='granted') {
              Geolocation.getCurrentPosition(info => {
                loc = {
                  latitude: info.coords.latitude,
                  longitude: info.coords.longitude,
                }
                setLocation(loc)
                setUserLocation(loc)
  
                mapRef.current.animateToRegion({
                  ...loc, 
                  latitudeDelta: 0.3,
                  longitudeDelta: 0.3,
                }, 1000)
              }).catch(e => console.log(e))
             }
             else{
               Alert.alert(
                null,
                'We need Premission',
                [
                  {
                    text: "Open Settings",
                    onPress: () => {
                      openSettings().catch(() => console.warn('cannot open settings'));
                    },
                    style: "cancel"
                  },
                  { text: "Close", onPress: () => {} }
                ],
                { cancelable: false }
              );
             }
            });
          }

        })
        .catch(e => console.log(e))
    } catch (error) {
      console.log(error)
    }

  }
  const circle = createRef(null);
  const mapRef = useRef(null);
 
    return(
      <View style={{backgroundColor:Colors_1.white, flex:1,}}>
      
            <View style={{
                borderRadius:4,  
                marginTop:height(4), alignSelf:'flex-start', flexDirection:'row'
                }}>
                <TouchableOpacity 
                  onPress={()=> navigation.goBack()}
                  onLongPress={()=> navigation.pop()}
                  style={{ alignSelf:'flex-start', marginHorizontal:width(3),backgroundColor:Colors_1.white, borderRadius:8,}}>
                    <Icon_1 name="arrow-left" size={30} color={Colors_1.gray} style={{alignSelf:'center', marginVertical:height(1.5)}} />
                </TouchableOpacity>
            </View>
            <MapView
              clusterColor={Colors_1.buttonBlue}
              style={{
                  height: "73%",
                  width: "100%"
                }}        
              onRegionChangeComplete={obj => {
                // setUserLocation(obj.nativeEvent.coordinate)
                console.log(obj)
                regionFrom(RADIUS)
                setLocation({ latitude: obj.latitude, longitude: obj.longitude })
                mapRef.current.animateToRegion({
                  ...obj
                  // latitude: obj.latitude, longitude: obj.longitude, 
                  // latitudeDelta: latDelta,
                  // longitudeDelta: lonDelta,
                }, 1000)
              }}
                ref={mapRef}
                showsUserLocation={false}
                zoomEnabled={true}
                loadingEnabled={false}
                provider={PROVIDER_GOOGLE}
                mapPadding={{ bottom: height(1) }}
                initialRegion={{
                latitude: userLocation.latitude,
                longitude: userLocation.longitude,
                latitudeDelta: latDelta,
                longitudeDelta: lonDelta,
              }}
            >
                <View style={{position: 'absolute', zIndex: 1, width:width(90),
                    backgroundColor:Colors_1.white, marginHorizontal:width(4), marginTop:height(2), borderRadius:6}}>
                    <Text style={{ zIndex: 1, marginHorizontal:width(3), marginVertical:height(0.5),
                        color:Colors_1.lightTextBlack,
                    }}>Where do you need a delivery?</Text>
                    <View style={{ zIndex: 1, flexDirection:'row', justifyContent:'space-between',}}>
                         <TextInput style={{marginVertical:height(1),
                            marginLeft:width(3),marginRight:width(1),
                        }} 
                        placeholder="Search for nearby area, street name" />
                       <TouchableOpacity style={{alignSelf:'center', zIndex: 1,}}>
                            <Icon_1 name="magnify" size={20} color={Colors_1.gray}
                            style={{alignSelf:"center",marginHorizontal:width(2)}} />
                        </TouchableOpacity>    
                    </View>
                </View>
           
                <Marker
                  cluster={false}
                  coordinate={userLocation}
                >
                  <Image
                      style={{ width: width(8), height: width(8), borderRadius: width(8) }}
                      source={{ uri: "data:image/jpeg;base64," }}
                  />
                </Marker>
                {tempMarker.length > 0 &&  tempMarker.map(marker => {
                  return (
                    <Marker
                      coordinate={marker.cordinate}
                      title={'Kisma Restaurant'}
                      description={''}
                      onPress={()=>navigation.navigate('ServiceDetailOverView')}
                      >
                      <Image
                        source={marker.marker}
                        style={{ height: 50, width: 50 }}
                        resizeMode="contain"
                      />
                    </Marker>
                  );
                })}
            </MapView> 

            <View style={{ position: 'absolute', zIndex: 1, bottom: height(15), alignSelf:"flex-end" }}>
              <TouchableOpacity onPress={getLocation
              } activeOpacity={0.8} style={{
                alignSelf:'flex-end',
                justifyContent: 'center', marginBottom: height(2), marginRight: width(5),
                alignItems: 'center', height:width(16),width:width(16), borderRadius: width(16)/2, backgroundColor: Colors_1.backgroundWhite,

              }} >
                <Image source={require('../../assets/buyerHome/navigation.png')} style={{ width: width(6.5), height: width(6.5), resizeMode: 'contain' }} />
              </TouchableOpacity>
            </View>   
                  
            <View style={{ flex:1, paddingHorizontal:width(3), zIndex: 1, }}>
                <Text style={{color:Colors_1.gray,paddingTop: 10,}}>Your Delivery Location</Text>
                <Text style={{}}>Sadguru Apartment</Text>
                <TouchableOpacity style={{borderWidth:1, borderRadius:4, backgroundColor:Colors_1.darkBackground, marginTop:height(1.5)}}>
                    <Text style={{color:Colors_1.white, alignSelf:"center", fontSize:totalSize(2), fontWeight:'bold', marginVertical:height(2),}}>Confirm Location</Text>
                </TouchableOpacity>
            </View>
            
      </View>
    )
}
export default NearMe_Screen;